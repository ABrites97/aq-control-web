...
const char* servidorAPI = "https://aq-control-web.vercel.app";

const char* esp32ApiKey = "31ge31rv3seg76se7v1sgsers3ger7g31wf5fwg6sv36serg8rse8vg767"; 
...
// Registo da memoria livre no historico - 2x por dia, a horas fixas
const int HORA_REGISTO_HEAP_1 = 3;
const int HORA_REGISTO_HEAP_2 = 15;
unsigned long ultimaChaveRegistoHeap = 0; // combina dia+hora, evita repetir o registo na mesma hora
...
bool radiadoresPausados = false;   // pausa manual - forca o rele 3 desligado independentemente do modo/temperatura// inicia desligado
...
function atualizarStatus(){
  ...
  .then(dados => {
    ...
    let radiadoresPausados = (v[13] == "1");
    let botaoPausa = document.getElementById("btnPausaRadiadores");
    if (radiadoresPausados){
      botaoPausa.innerText = "▶️ Retomar Radiadores";
      botaoPausa.classList.add("ativo");
    } else {
      botaoPausa.innerText = "⏸️ Pausar Radiadores";
      botaoPausa.classList.remove("ativo");
    }
...
function alternarPausaRadiadores(){

  let pausadoAgora = document.getElementById("btnPausaRadiadores").classList.contains("ativo");
  let novoValor = pausadoAgora ? "0" : "1";

  fetch('/radiadores_pausa?valor=' + novoValor)
...
<button id="btnPausaRadiadores" onclick="alternarPausaRadiadores()" style="width:90%;">
⏸️ Pausar Radiadores
</button>
...
<div class="linha">
<span>Bomba Caldeira - Mínima:</span>
...
<div class="linha">
<span>Bomba Caldeira - Limite p/ desligar:</span>
<div class="campo-temp">
...
server.on("/radiadores_pausa", [](){

  if (server.hasArg("valor")){
    radiadoresPausados = (server.arg("valor") == "1");
    registarEvento(radiadoresPausados ? "Radiadores: pausados manualmente" : "Radiadores: retomado o funcionamento automatico");
  }

  server.send(200, "text/plain", String(radiadoresPausados));
...
server.on("/temp_config", [](){
  ...
  if (server.hasArg("t1liga") && server.hasArg("t1desliga")
    ...
    // >>> VALIDAÇÃO — Bomba Caldeira: minimo para ligar tem de ser menor que o limite de desligar
    if (novoT1Liga >= novoT1Desliga){
      server.send(400, "text/plain", "Erro: o valor de ligar tem de ser menor que o de desligar (Bomba Caldeira)");
      return;
    }
...
void controlarRele2(){
  ...
  if (rele1Ligado){
    // Enquanto a Ordem Caldeira (rele 1) estiver ligada, a bomba so pode LIGAR
    // (espera pelo minimo) e depois fica sempre ligada - nunca desliga aqui.
    ...
    // A Ordem Caldeira (rele 1) esta desligada - a bomba (se estiver ligada)
    // so desliga quando a caldeira arrefecer abaixo do limite de desligar.
...
void controlarRele3(){
  ...
  if (radiadoresPausados){
    if (rele3Ligado){
      rele3Ligado = false;
      digitalWrite(RELE3_PIN, HIGH);
      registarEvento("Bomba Aquecimento: OFF (pausado manualmente)");
    }
    return;
  }
...
void enviarLeituraVercel(){
  ...
  String url = String(servidorAPI) + "/api/leituras";
  ...
  json += "\"tempCaldeira\":" + String(tempCaldeira,1) + ",";
  json += "\"tempAQS\":" + String(tempAQS,1) + ",";

  json += "\"rele1Ligado\":" + String(rele1Ligado ? "true":"false") + ",";
  json += "\"rele2Ligado\":" + String(rele2Ligado ? "true":"false") + ",";
  json += "\"rele3Ligado\":" + String(rele3Ligado ? "true":"false") + ",";

  json += "\"modo\":\"" + modoAtual + "\",";
  json += "\"radiadoresPausados\":" + String(radiadoresPausados ? "true":"false");
...
if(millis() - ultimoEnvioAPI > 30000){
  ...
  // Regista a memoria livre 2x por dia (3h e 15h) - ajuda a diagnosticar se o sistema
  // ficar lento/preso outra vez (fragmentacao de memoria a esgotar-se), sem encher o
  // historico de eventos com registos horarios desnecessarios
  if (rtcOK){
    DateTime agoraHeap = rtc.now();
    if (agoraHeap.hour() == HORA_REGISTO_HEAP_1 || agoraHeap.hour() == HORA_REGISTO_HEAP_2){
      unsigned long chaveAtual = (agoraHeap.unixtime() / 86400UL) * 100UL + agoraHeap.hour();
      if (chaveAtual != ultimaChaveRegistoHeap){
        ultimaChaveRegistoHeap = chaveAtual;

...
const unsigned long TEMPO_ECRA_LCD = 5000;      // troca de ecrã a cada 5s
...
function atualizarStatus(){
  ...
  .then(dados => {
    ...
    document.getElementById("tempCaldeira").innerHTML = v[2] + " °C";
    document.getElementById("tempAQS").innerHTML = v[3] + " °C";
...
server.on("/temp_config", [](){
  ...
  if (server.hasArg("t1liga") && server.hasArg("t1desliga")
    ...
    temp1Liga    = server.arg("t1liga").toFloat();
    temp1Desliga = server.arg("t1desliga").toFloat();
    temp2Liga    = server.arg("t2liga").toFloat();
    temp2Desliga = server.arg("t2desliga").toFloat();
...
void desenharEcraLCD(){
  ...
  if (ecraLCD == 0){
    lcdLinha(0, " Hora: " + horaAtual);
    lcdLinha(1, " Modo: " + modoAtual);
  }
  else if (ecraLCD == 1){
    lcdLinha(0, "Temp.Cal: " + String(tempCaldeira,1) + (char)223 + "C");
    lcdLinha(1, "Temp.AQS: " + String(tempAQS,1) + (char)223 + "C");
  }
  else if (ecraLCD == 2){

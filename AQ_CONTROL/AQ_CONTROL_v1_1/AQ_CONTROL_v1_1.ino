...
bool sincronizarRTCcomNTP(){
  ...
  // Diagnóstico: confirma se o DNS consegue resolver o servidor NTP.
  // Isto distingue "falha de DNS" de "falha de ligação NTP (porta UDP 123 bloqueada)".
  IPAddress ipTeste;
  if (WiFi.hostByName("pool.ntp.org", ipTeste)){
    Serial.print("DNS OK - pool.ntp.org resolvido para: ");
    Serial.println(ipTeste);
    registarEvento("Diagnostico NTP: DNS OK (" + ipTeste.toString() + ")");
  } else {
    Serial.println("DNS FALHOU a resolver pool.ntp.org");
  ...
  for (int tentativa = 1; tentativa <= MAX_TENTATIVAS && !obteveHora; tentativa++){
    // getLocalTime tenta durante o tempo indicado (ms) até a hora estar disponível
    obteveHora = getLocalTime(&timeinfo, 5000);
    if (!obteveHora && tentativa < MAX_TENTATIVAS){
      delay(2000);   // dá tempo à rede/servidor NTP antes da próxima tentativa
    }
  }

  if (!obteveHora){
    registarEvento("Falha ao sincronizar hora via NTP (apos " + String(MAX_TENTATIVAS) + " tentativas)");
    return false;
  }
